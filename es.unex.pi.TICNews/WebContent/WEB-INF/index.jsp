...
<body>
	...
	<main> <c:forEach var="newsmap" items="${newsMap}">
		<article>
			...
			<p>
				enviado por <a href="<c:url value='UserDetailServlet?id=${newsmap.value.id}'/>" class="enviadoPor">${newsmap.value.name}</a>:
			</p>
			<p>${newsmap.key.text}</p>

			<div class="feedback">

				<span class="comentarios"><a href="<c:url value='ListCommentsServlet?id=${newsmap.key.id}'/>">comentarios</a></span>
